...
class MainDBSettingsUpdate(BaseModel):
    ...
    main_db_name: str
    corp_table_name: str
    corp_code_column: str
    corp_name_column: str
    biz_no_column: str
    repr_name_column: str
    acc_db_name_column: str
...
# ============================================================
# Main DB Settings (법인 정보 조회용)
# ============================================================

@router.get("/main-db")
async def get_main_db_settings(db: Session = Depends(get_db)):
    """메인 DB 설정 조회"""
    server_id = get_config_value(db, "main_db_server_id", "")
    return {
