...
def get_context(request: Request, user, **kwargs):
    ...
    server_id가 있고 current_server가 있으면 server_name을 자동 추가
    (사이드바에서 선택된 서버명 표시용)
    """
    ctx = {
        "request": request,
    ...
    # current_server 객체가 있으면 server_name 자동 설정
    current_server = kwargs.get("current_server")
    if current_server and "server_name" not in kwargs:
        ctx["server_name"] = current_server.server_name
    return ctx
...
async def servers_page(
    ...
    # next 파라미터 검증: 내부 경로만 허용 (open redirect 방지)
    safe_next = None
...
@router.get("/schema-export/{server_id}", response_class=HTMLResponse)
async def schema_export_page(
    ...
    return templates.TemplateResponse("pages/schema_export.html", get_context(
        ...
        current_server=current_server,
        server_id=server_id,
        selected_server_id=server_id    # 기존 템플릿 호환
    ))

@router.get("/activity-logs", response_class=HTMLResponse)
@router.get("/activity-logs/{server_id}", response_class=HTMLResponse)
