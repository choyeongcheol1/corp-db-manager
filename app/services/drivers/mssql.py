...
class MSSQLDriver(BaseDriver):
    ...
    def get_connection(self, database: str = None, max_retries: int = 5) -> pyodbc.Connection:
        """DB 연결 획득 (재시도 포함)"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait = (attempt + 1) * 2  # 2초, 4초, 6초, 8초 점진적 대기
                    print(f"[MSSQL] 연결 재시도 ({attempt + 1}/{max_retries}, {wait}초 후): {self.server.host} / {database}")
                    time.sleep(wait)
